...
#   1. $ROOT/nickel-compose/nickel-compose.ncl  (submodule layout)
#   2. $ROOT/nickel-compose.ncl                 (vendored layout)
#   3. $SCRIPT_DIR/../nickel-compose.ncl        (alongside the script,
...
MERGE_LIB=""
for candidate in \
    "$ROOT/nickel-compose/nickel-compose.ncl" \
    "$ROOT/nickel-compose.ncl" \
    "$NC_ROOT/nickel-compose.ncl"; do
  if [[ -f "$candidate" ]]; then
...
if [[ -z "$MERGE_LIB" ]]; then
  ...
  if [[ "$OUT_DIR" == "$ROOT" ]]; then
    MERGE_LIB="nickel-compose/nickel-compose.ncl"
  else
    MERGE_LIB="../nickel-compose/nickel-compose.ncl"
  fi
fi
...

{
  ...
  echo ""
  echo "let composer = import \"$MERGE_LIB\" in"
  echo ""
  echo "let fragments = ["
