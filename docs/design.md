...
| No typecheck | Contracts. Missing field → typecheck error with file:line, before any container starts. Per-fragment contracts as needed (`GoclawBase`, `PostgresBase`, etc.). `mise run check` runs the strict typecheck. Schema validation (the `x-check` report) runs as part of `merge_fully_validate` and is recorded in the artifact; `use` does not fail on schema errors. |
...
`check` function and `merge_fully_validate` (with `x-check`
field carrying the schema report and `x-source` carrying the
literal input path; `x-*` is a Compose extension field that
the runtime silently ignores), and the wrapper integration
(`bin/nickel-compose-use.sh` runs `merge_fully_validate`,
writes both `compose.ncl` and `compose.yaml`, exits 0 on
successful render regardless of `x-check.ok`) are all working. `mise run check` runs the strict typecheck. The
